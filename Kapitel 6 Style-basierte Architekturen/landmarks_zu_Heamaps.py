...
# def landmarks_to_heatmaps(landmarks, H=64, W=64, sigma=2.5, device="cpu"):
#     """
#     Convert landmark coordinates (x,y in [0,1]) to Gaussian heatmaps.
#     landmarks: np.ndarray (N,2)
#     """
#     if landmarks is None or len(landmarks) == 0:
#         return np.zeros((0, H, W), dtype=np.float32)
#
#     pts = torch.tensor(landmarks, device=device)
#
#     # Normalize w.r.t bounding box of subset
...
if __name__ == "__main__":
    ...
    #Example usages for visualization and heatmap generation
    #Sanity Checks

    npz_path = r"C:\Users\pnieg\Documents\HDTF\HDTF_dataset_test\__lRwnjxeCg_2\__lRwnjxeCg_2_clip_0000\landmarks\landmarks_0015.npz"
    visualize_saved_landmarks(npz_path)
    visualize_landmarks_on_image(
        npz_path,
        r"C:\Users\pnieg\Documents\HDTF\HDTF_dataset_test\__lRwnjxeCg_2\__lRwnjxeCg_2_clip_0000\identity\0015.jpg"
    )
    #
    data = np.load(npz_path)
    #
    expr = data["expr"]  # (N_expr, 2)
    # #expr = np.concatenate([expr[:54], expr[87:]], axis=0)
    pose = data["pose"]  # (N_pose, 2)
    # expr_and_pose = np.concatenate([expr, pose], axis=0)
    mouth = data["mouth"]  # (N_mouth, 2)

    expr = rotate_landmarks(expr, angle_deg=15)

    expr_sliced = np.concatenate([
    expr[:36],      # everything before right outer eye
    expr[53:70],    # between right and left outer eye
    expr[86:]       # everything after left outer eye
])

    mouth = rotate_landmarks(mouth, angle_deg=-10)

    #
    # # Rotate landmarks for testing
    # #expr_rotated = rotate_landmarks(expr, angle_deg=25)
    # #mouth_rotated = rotate_landmarks(mouth, angle_deg=-25)
    #
    #
    pose_hm = landmarks_to_heatmaps_refined(torch.tensor(pose), mode='absolute', H=128, W=128, sigma=2.5)
    expr_hm = landmarks_to_heatmaps_refined(torch.tensor(expr), mode='mouth', H=128, W=128, sigma=2)
    mouth_hm = landmarks_to_heatmaps_refined(torch.tensor(mouth), mode='mouth',  H=128, W=128, sigma=2.5)
    expr_sliced_hm = landmarks_to_heatmaps_refined(torch.tensor(expr_sliced), mode='mouth', H=128, W=128, sigma=2.5)
    # expr_and_pose_hm = landmarks_to_heatmaps_refined(torch.tensor(expr_and_pose), mode='absolute', H=128, W=128, sigma=2.5)
    # #expr_hm_rot = landmarks_to_heatmaps_refined(torch.tensor(expr_rotated), mode='mouth')
    # #mouth_hm_rot = landmarks_to_heatmaps_refined(torch.tensor(mouth_rotated), mode='mouth')
    # print("Pose heatmaps shape:", pose_hm.shape)
    # print("Expression heatmaps shape:", expr_hm.shape)
    # print("Mouth heatmaps shape:", mouth_hm.shape)
    # print("Expr + Pose heatmaps shape:", expr_and_pose_hm.shape)
    # #print("Rotated Expression heatmaps shape:", expr_hm_rot.shape)
    # #print("Rotated Mouth heatmaps shape:", mouth_hm_rot.shape)
    #
    print(expr_sliced_hm.shape)
    print(pose_hm.shape)
    visualize_single_heatmap(pose_hm, title="Pose Heatmaps", colormap='jet')
    visualize_single_heatmap(expr_hm, title="Expression Heatmaps", colormap='jet')
    visualize_single_heatmap(mouth_hm, title="Mouth Heatmaps", colormap='jet')
    visualize_single_heatmap(expr_sliced_hm, title="Sliced Expression Landmarks", colormap='jet')
    # visualize_single_heatmap(expr_and_pose_hm, title="Expr + Pose Heatmaps", colormap='jet')




    # Example usage:
    #Visualize a saved heatmap
    #npz_path_expr = r"C:\Users\pnieg\Documents\HDTF\HDTF_dataset_test\_5ukjsqqLg4_9\_5ukjsqqLg4_9_clip_0000\heatmaps\0000_pose_and_expr.npz"
    #npz_path_mouth = r"C:\Users\pnieg\Documents\HDTF\HDTF_dataset_test\_5ukjsqqLg4_9\_5ukjsqqLg4_9_clip_0000\heatmaps\0000_mouth.npz"
    #npz_path_pose = r"C:\Users\pnieg\Documents\HDTF\HDTF_output_test2\onacY99veEQ_1\onacY99veEQ_1_clip_0000\heatmaps\0000_pose.npz"

    #visualize_saved_heatmap(npz_path_expr)
    #visualize_saved_heatmap(npz_path_mouth)
    # #visualize_saved_heatmap(npz_path_pose)
    #start = time.time()

    #input_root = r"C:\Users\pnieg\Documents\HDTF\HDTF_dataset"
    #output_root = r"C:\Users\pnieg\Documents\Masterarbeit\TalkingHead-1KH\heatmaps"
    #print("Processing all landmarks to heatmaps...")
    #process_all_landmarks(input_root, num_workers=4)
    #print("Done.")
    #end = time.time()
